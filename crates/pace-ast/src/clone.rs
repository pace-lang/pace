use crate::{Expr, Stmt, arena::{AstArena, ExprId, StmtId}};
    ...
    pub fn deep_clone_expr(&mut self, expr_id: ExprId) -> ExprId {
        ...
        let cloned_expr = match expr {
            ...
            Expr::GenericInstantiation { callee, generic_args } => {
                ...
                Expr::GenericInstantiation { callee: new_callee, generic_args }
            }
            Expr::Binary { left, op, right } => {
                let new_left = self.deep_clone_expr(left);
                let new_right = self.deep_clone_expr(right);
                Expr::Binary { left: new_left, op, right: new_right }
            }
            Expr::Call { callee, args } => {
                let new_callee = self.deep_clone_expr(callee);
                let new_args = args.into_iter().map(|arg| self.deep_clone_expr(arg)).collect();
            ...
            Expr::Assign { target, value } => {
                ...
                Expr::Assign { target: new_target, value: new_value }
            }
            Expr::MemberAccess { object, property, computed_class, is_static_operator } => {
                let new_object = self.deep_clone_expr(object);
                Expr::MemberAccess { object: new_object, property, computed_class, is_static_operator }
            }
            Expr::Unwrap(expr) => {
                Expr::Unwrap(self.deep_clone_expr(expr))
            }
            Expr::OptionalMemberAccess { object, property } => {
                let new_object = self.deep_clone_expr(object);
                Expr::OptionalMemberAccess { object: new_object, property }
            }
            Expr::NullCoalesce { left, right } => {
                let new_left = self.deep_clone_expr(left);
                let new_right = self.deep_clone_expr(right);
                Expr::NullCoalesce { left: new_left, right: new_right }
            }
            Expr::Try(expr) => {
                Expr::Try(self.deep_clone_expr(expr))
            }
            Expr::Await(expr) => {
                Expr::Await(self.deep_clone_expr(expr))
            }
            Expr::Closure { params, return_type, body } => {
                let new_body = self.deep_clone_expr(body);
                Expr::Closure { params, return_type, body: new_body }
            }
            Expr::Block(stmts) => {
    ...
    pub fn deep_clone_stmt(&mut self, stmt_id: StmtId) -> StmtId {
        ...
        let cloned_stmt = match stmt {
            ...
            Stmt::VarDecl { name, is_mutable, type_annotation, is_static, visibility, initializer, span } => {
                ...
                Stmt::VarDecl { name, is_mutable, type_annotation, is_static, visibility, initializer: new_init, span }
            }
            Stmt::Block(stmts) => {
                let new_stmts = stmts.into_iter().map(|s| self.deep_clone_stmt(s)).collect();
            ...
            Stmt::If { condition, then_branch, else_branch } => {
                ...
                Stmt::If { condition: new_cond, then_branch: new_then, else_branch: new_else }
            }
            Stmt::While { condition, body } => {
                let new_cond = self.deep_clone_expr(condition);
                let new_body = self.deep_clone_stmt(body);
                Stmt::While { condition: new_cond, body: new_body }
            }
            Stmt::Loop { body } => {
                let new_body = self.deep_clone_stmt(body);
            ...
            Stmt::ForIn { item, iterable, body } => {
                ...
                let new_body = self.deep_clone_stmt(body);
                Stmt::ForIn { item, iterable: new_iter, body: new_body }
            }
            Stmt::FuncDecl { name, generic_params, params, return_type, body, is_async, is_static, visibility, doc_comment, span } => {
                ...
                Stmt::FuncDecl { name, generic_params, params, return_type, body: new_body, is_async, is_static, visibility, doc_comment, span }
            }
            Stmt::ClassDecl { name, generic_params, fields, methods, implements, doc_comment } => {
                let new_fields = fields.into_iter().map(|f| self.deep_clone_stmt(f)).collect();
                let new_methods = methods.into_iter().map(|m| self.deep_clone_stmt(m)).collect();
