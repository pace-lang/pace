...
impl<'a> EscapeAnalysis<'a> {
    pub fn new(arena: &'a pace_ast::arena::AstArena) -> Self {
        Self {
            arena,
    ...
    fn visit_stmt(&mut self, stmt: &pace_ast::Stmt) {
        match stmt {
            Stmt::VarDecl {
                name, initializer, ..
            } => {
            ...
            Stmt::Block(stmts) => {
                self.push_scope();
                for s in stmts {
                    self.visit_stmt(self.arena.get_stmt(*s));
                }
                self.pop_scope();
            }
            ...
            Stmt::If {
                ...
                self.visit_stmt(self.arena.get_stmt(*then_branch));
                if let Some(e) = else_branch {
                    self.visit_stmt(self.arena.get_stmt(*e));
                }
            }
            Stmt::While { condition, body } => {
                self.visit_expr(*condition);
                self.visit_stmt(self.arena.get_stmt(*body));
            }
            Stmt::Loop { body } => {
                self.visit_stmt(self.arena.get_stmt(*body));
            }
    ...
    fn visit_expr(&mut self, expr_id: pace_ast::arena::ExprId) {
