...
impl Inliner {
    ...
    fn extract_inlineable(&mut self, arena: &pace_ast::arena::AstArena, ast: &[pace_ast::arena::StmtId]) {
        for &stmt_id in ast {
    ...
    fn get_single_expression(&self, arena: &pace_ast::arena::AstArena, body: &[pace_ast::arena::StmtId]) -> Option<pace_ast::arena::ExprId> {
        if body.len() == 1 {
    ...
    fn rewrite_stmt(&mut self, arena: &mut pace_ast::arena::AstArena, stmt_id: pace_ast::arena::StmtId) -> pace_ast::arena::StmtId {
        let stmt = arena.get_stmt(stmt_id).clone();
        let new_stmt = match stmt {
            Stmt::Module { name, body } => {
                let new_body = body.into_iter().map(|s| self.rewrite_stmt(arena, s)).collect();
            ...
            Stmt::ClassDecl { name, generic_params, fields, methods, implements, doc_comment } => {
                let new_fields = fields.into_iter().map(|f| self.rewrite_stmt(arena, f)).collect();
                let new_methods = methods.into_iter().map(|m| self.rewrite_stmt(arena, m)).collect();
            ...
            Stmt::Block(stmts) => {
                Stmt::Block(stmts.into_iter().map(|s| self.rewrite_stmt(arena, s)).collect())
            }
            ...
            Stmt::Match { expr, arms } => {
                let new_arms = arms.into_iter().map(|(pat, body)| (pat, self.rewrite_stmt(arena, body))).collect();
                Stmt::Match {
    ...
    fn rewrite_expr(&mut self, arena: &mut pace_ast::arena::AstArena, expr_id: pace_ast::arena::ExprId) -> pace_ast::arena::ExprId {
        let expr = arena.get_expr(expr_id).clone();
        let new_expr = match expr {
            Expr::Call { callee, args } => {
                let new_callee = self.rewrite_expr(arena, callee);
                let new_args: Vec<pace_ast::arena::ExprId> = args.into_iter().map(|a| self.rewrite_expr(arena, a)).collect();
                ...
                if let Expr::Identifier(name, _) = arena.get_expr(new_callee)
                    && let Some((param_names, body_expr)) = self.inlineable.get(name).cloned() {
                        ...
                        if new_args.iter().all(|arg| self.is_safe_argument(arena, *arg)) && new_args.len() == param_names.len() {
                            return self.inline_call(arena, body_expr, &param_names, &new_args);
                        }
                    }
                
                Expr::Call { callee: new_callee, args: new_args }
            }
            ...
            Expr::MemberAccess { object, property, computed_class, is_static_operator } => {
                Expr::MemberAccess {
                    ...
                    property, computed_class, is_static_operator
                }
            }
            ...
            Expr::InterpolatedString(parts) => {
                Expr::InterpolatedString(parts.into_iter().map(|p| self.rewrite_expr(arena, p)).collect())
            }
            ...
            Expr::Block(stmts) => {
                Expr::Block(stmts.into_iter().map(|s| self.rewrite_stmt(arena, s)).collect())
            }
    ...
    fn is_safe_argument(&self, arena: &pace_ast::arena::AstArena, arg: pace_ast::arena::ExprId) -> bool {
    ...
    fn inline_call(&mut self, arena: &mut pace_ast::arena::AstArena, body: pace_ast::arena::ExprId, param_names: &[ustr::Ustr], args: &[pace_ast::arena::ExprId]) -> pace_ast::arena::ExprId {
    ...
    fn substitute_expr(&self, arena: &mut pace_ast::arena::AstArena, expr_id: pace_ast::arena::ExprId, subs: &HashMap<ustr::Ustr, pace_ast::arena::ExprId>) {
    ...
    fn substitute_stmt(&self, arena: &mut pace_ast::arena::AstArena, stmt_id: pace_ast::arena::StmtId, subs: &std::collections::HashMap<ustr::Ustr, pace_ast::arena::ExprId>) {
        ...
        match &mut stmt {
            ...
            Stmt::VarDecl { initializer: Some(expr), .. } => self.substitute_expr(arena, *expr, subs),
            Stmt::If { condition, then_branch, else_branch } => {
