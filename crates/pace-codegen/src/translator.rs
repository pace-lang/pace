...
use cranelift_module::{Module, DataDescription, Linkage};
use pace_ast::{Expr, Stmt, BinaryOp};
use std::collections::HashMap;
use crate::layouts::{StructLayout, EnumLayout, CodegenError};
...
impl<'a, 'b, M: Module> Translator<'a, 'b, M> {
        ...
        match stmt {
            ...
            Stmt::Expr(expr) => {
                ...
                if matches!(ty, VarType::Object(_)) {
                    ...
                    let local_release = self.context.module.declare_func_in_func(release_id, self.builder.func);
            ...
            Stmt::Match { expr, arms } => {
                ...
                let tag_val = self.builder.ins().load(types::I64, cranelift::prelude::MemFlagsData::new(), obj_ptr, 8);
                ...
                for (i, (pattern, _)) in arms.iter().enumerate() {
                    if let pace_ast::Pattern::Variant { variant_name, .. } = pattern {
                        ...
                        let is_match = self.builder.ins().icmp(cranelift::codegen::ir::condcodes::IntCC::Equal, tag_val, expected_tag);
                        self.builder.ins().brif(is_match, blocks[i], &[], next_check, &[]);
                        ...
                        if let Some(pat_fields) = pat_fields {
                            ...
                            for (j, pat) in pat_fields.iter().enumerate() {
                                if let pace_ast::Pattern::Variable(var_name, _span) = pat {
                                    let field_val = self.builder.ins().load(types::I64, cranelift::prelude::MemFlagsData::new(), obj_ptr, offset);
                                    let var = self.builder.declare_var(types::I64);
                                    self.builder.def_var(var, field_val);
                                    self.variables.insert(var_name.clone(), (var, variant_types[j].clone()));
                                    *self.var_index += 1;
                                }
                                offset += 8;
                            }
            ...
            Stmt::ForIn { item, iterable, body, .. } => {
                ...
                // Get length
                let vtable_ptr_len = self.builder.ins().load(ptr_ty, cranelift::prelude::MemFlagsData::new(), iter_val, 8);
                let method_ptr_len = self.builder.ins().load(ptr_ty, cranelift::prelude::MemFlagsData::new(), vtable_ptr_len, length_offset as i32);
                ...
                let callee_len = self.builder.ins().call_indirect(sig_len_ref, method_ptr_len, &[iter_val]);
                let len_val = self.builder.inst_results(callee_len)[0];
                
                let curr_idx = self.builder.use_var(idx_var);
                let is_less = self.builder.ins().icmp(cranelift::codegen::ir::condcodes::IntCC::SignedLessThan, curr_idx, len_val);
                self.builder.ins().brif(is_less, body_block, &[], exit_block, &[]);
                
                self.builder.seal_block(body_block);
                self.builder.switch_to_block(body_block);
                
                // Get item
                let vtable_ptr_get = self.builder.ins().load(ptr_ty, cranelift::prelude::MemFlagsData::new(), iter_val, 8);
                let method_ptr_get = self.builder.ins().load(ptr_ty, cranelift::prelude::MemFlagsData::new(), vtable_ptr_get, get_offset as i32);
                ...
                let callee_get = self.builder.ins().call_indirect(sig_get_ref, method_ptr_get, &[iter_val, curr_idx]);
                ...
                self.variables.insert(item.clone(), (item_var, VarType::Unknown));
                *self.var_index += 1;
                    ...
                    for var_name in current_vars {
                        if !initial_vars.contains(&var_name) {
                            ...
                            if matches!(ty, VarType::Object(_)) {
                                ...
                                let release_id = *self.context.funcs.get("release").unwrap_or_else(|| panic!("release not found"));
                                let local_release = self.context.module.declare_func_in_func(release_id, self.builder.func);
        ...
        match expr {
            ...
            Expr::Call { callee, .. } => {
                if let Expr::Identifier(name) = &**callee {
                    ...
                    if let Expr::Identifier(obj_name) = &**object {
                        ...
                        if self.context.class_layouts.contains_key(obj_name) || self.context.struct_layouts.contains_key(obj_name) {
            ...
            Expr::Try(inner) => {
                ...
                if let VarType::Enum(name) = inner_ty {
                    if name.starts_with("Result_") {
                        ...
                        if parts.len() >= 3 {
                            return parse_vartype(parts[1], None, Some(&self.context.struct_layouts), None);
                        }
            ...
            Expr::Closure { params, return_type, .. } => {
                ...
                for (_, ty_ann) in params {
                    param_types.push(parse_type_annotation(ty_ann, None, Some(&self.context.struct_layouts), None));
                }
                let ret = if let Some(ret_ann) = return_type {
                    Box::new(parse_type_annotation(ret_ann, None, Some(&self.context.struct_layouts), None))
                } else {
                    Box::new(VarType::Unknown)
                };
    ...
    fn copy_struct(&mut self, name: &str, src_ptr: Value) -> Value {
        ...
        for &(offset, _) in layout.fields.values() {
            let field_val = self.builder.ins().load(types::I64, cranelift::prelude::MemFlagsData::new(), src_ptr, offset as i32);
            self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), field_val, dst_ptr, offset as i32);
        }
        ...
        match expr {
            ...
            Expr::StringLiteral(s) => {
                ...
                let string_name = if let Some(name) = self.context.string_cache.get(s) {
                    ...
                    let id = self.context.string_id; self.context.string_id += 1;
                    ...
                    let data_id = self.context.module.declare_data(&name, Linkage::Local, false, false).unwrap();
                ...
                let data_id = self.context.module.declare_data(&string_name, Linkage::Local, false, false).unwrap();
                let local_id = self.context.module.declare_data_in_func(data_id, self.builder.func);
                let ptr_ty = self.context.module.target_config().pointer_type();
                ...
                for part in parts {
                    ...
                    let str_val = if part_ty == VarType::String {
                        ...
                        if self.builder.func.dfg.value_type(val) == types::I64 {
                            float_val = self.builder.ins().bitcast(types::F64, cranelift::prelude::MemFlagsData::new(), val);
                        }
                        let to_str = self.context.module.declare_func_in_func(*self.context.funcs.get("float_to_string").unwrap(), self.builder.func);
                        let call = self.builder.ins().call(to_str, &[float_val]);
                        self.builder.inst_results(call)[0]
                    } else if part_ty == VarType::Bool {
                        let to_str = self.context.module.declare_func_in_func(*self.context.funcs.get("bool_to_string").unwrap(), self.builder.func);
                        let call = self.builder.ins().call(to_str, &[val]);
                        self.builder.inst_results(call)[0]
                    } else { // Assume Int
                        let to_str = self.context.module.declare_func_in_func(*self.context.funcs.get("int_to_string").unwrap(), self.builder.func);
                        let call = self.builder.ins().call(to_str, &[val]);
                        self.builder.inst_results(call)[0]
                    };
                    
                    if let Some(prev) = current_val {
                        let concat = self.context.module.declare_func_in_func(*self.context.funcs.get("concat_strings").unwrap(), self.builder.func);
            ...
            Expr::Identifier(name) => {
                if let Some((var, ty)) = self.variables.get(name) {
                    ...
                    if matches!(ty, VarType::Object(_)) {
                        ...
                        let local_retain = self.context.module.declare_func_in_func(retain_id, self.builder.func);
            ...
            Expr::Binary { left, op, right } => {
                ...
                match op {
                    BinaryOp::Add => {
                        if is_float { Ok(self.builder.ins().fadd(lhs, rhs)) } else { Ok(self.builder.ins().iadd(lhs, rhs)) }
                    }
                    BinaryOp::Sub => {
                        if is_float { Ok(self.builder.ins().fsub(lhs, rhs)) } else { Ok(self.builder.ins().isub(lhs, rhs)) }
                    }
                    BinaryOp::Mul => {
                        if is_float { Ok(self.builder.ins().fmul(lhs, rhs)) } else { Ok(self.builder.ins().imul(lhs, rhs)) }
                    }
                    BinaryOp::Div => {
                        if is_float { Ok(self.builder.ins().fdiv(lhs, rhs)) } else { Ok(self.builder.ins().sdiv(lhs, rhs)) }
                    }
                    BinaryOp::Mod => {
                        if is_float {
                    ...
                    BinaryOp::LessEq => {
                        if is_float {
                            ...
                            let c = self.builder.ins().icmp(IntCC::SignedLessThanOrEqual, lhs, rhs);
                            Ok(self.builder.ins().uextend(types::I64, c))
                        }
                    }
                    BinaryOp::Greater => {
                    ...
                    BinaryOp::GreaterEq => {
                        if is_float {
                            let c = self.builder.ins().fcmp(FloatCC::GreaterThanOrEqual, lhs, rhs);
                            Ok(self.builder.ins().uextend(types::I64, c))
                        } else {
                            let c = self.builder.ins().icmp(IntCC::SignedGreaterThanOrEqual, lhs, rhs);
                            Ok(self.builder.ins().uextend(types::I64, c))
                        }
                    }
                    BinaryOp::And => {
            ...
            Expr::Await(inner) => {
                ...
                let local_await = self.context.module.declare_func_in_func(await_id, self.builder.func);
            ...
            Expr::Try(inner) => {
                ...
                let tag_val = self.builder.ins().load(types::I64, cranelift::prelude::MemFlagsData::new(), inner_ptr, 8);
                let expected_tag = self.builder.ins().iconst(types::I64, *success_tag as i64);
                let is_success = self.builder.ins().icmp(cranelift::codegen::ir::condcodes::IntCC::Equal, tag_val, expected_tag);
                
                let continue_block = self.builder.create_block();
                let err_block = self.builder.create_block();
                
                self.builder.ins().brif(is_success, continue_block, &[], err_block, &[]);
                ...
                let val = self.builder.ins().load(types::I64, cranelift::prelude::MemFlagsData::new(), inner_ptr, 16);
                Ok(val)
            }
            Expr::Assign { target, value } => {
                ...
                if let Expr::Identifier(name) = &**target {
                    ...
                    if let Expr::Identifier(obj_name) = &**object {
                        let maybe_static_field = if let Some(layout) = self.context.class_layouts.get(obj_name) {
                        ...
                        if let Some(&(data_id, ref f_ty)) = maybe_static_field {
                            let ptr_ty = self.context.module.target_config().pointer_type();
                            let data_ref = self.context.module.declare_data_in_func(data_id, self.builder.func);
                            let addr = self.builder.ins().symbol_value(ptr_ty, data_ref);
                            
                            if matches!(f_ty, VarType::Object(_)) {
                                let old_val = self.builder.ins().load(f_ty.to_cranelift_type(), cranelift::prelude::MemFlagsData::new(), addr, 0);
                                let release_id = *self.context.funcs.get("release").unwrap();
                                let local_release = self.context.module.declare_func_in_func(release_id, self.builder.func);
                                self.builder.ins().call(local_release, &[old_val]);
                            ...
                            self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), val, addr, 0);
                            return Ok(val);
                        }
                    }
                    ...
                    if matches!(f_ty, VarType::Object(_)) {
                        let old_val = self.builder.ins().load(f_ty.to_cranelift_type(), cranelift::prelude::MemFlagsData::new(), obj_ptr, f_offset as i32);
                        let release_id = *self.context.funcs.get("release").unwrap();
                        let local_release = self.context.module.declare_func_in_func(release_id, self.builder.func);
                        self.builder.ins().call(local_release, &[old_val]);
                    ...
                    self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), val, obj_ptr, f_offset as i32);
                    Ok(val)
                } else {
            ...
            Expr::Call { callee, args } => {
                ...
                if matches!(callee_ty, VarType::Function(_, _)) {
                    ...
                    let ptr_ty = self.context.module.target_config().pointer_type();
                    let func_ptr = self.builder.ins().load(ptr_ty, cranelift::prelude::MemFlagsData::new(), fat_ptr, 0);
                ...
                if let Expr::Identifier(func_name) = &**callee {
                    if func_name == "print" {
                        ...
                        let local_func = self.context.module.declare_func_in_func(func_id, self.builder.func);
                        ...
                        let local_malloc = self.context.module.declare_func_in_func(malloc_id, self.builder.func);
                        ...
                        // Set ARC count to 1
                        let one = self.builder.ins().iconst(types::I64, 1);
                        self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), one, obj_ptr, 0);
                        
                        // Set VTable
                        let vtable_gv = self.context.module.declare_data_in_func(layout.vtable_id, self.builder.func);
                        let vtable_addr = self.builder.ins().symbol_value(ptr_ty, vtable_gv);
                        self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), vtable_addr, obj_ptr, 8);
                        
                        let zero = self.builder.ins().iconst(types::I64, 0);
                        for (field_name, &(offset, _)) in &layout.fields {
                            if field_name == "__mailbox" {
                                let mb_create_id = *self.context.funcs.get("__pace_mailbox_create").unwrap();
                                let local_mb_create = self.context.module.declare_func_in_func(mb_create_id, self.builder.func);
                                let mb_call = self.builder.ins().call(local_mb_create, &[]);
                                let mb_ptr = self.builder.inst_results(mb_call)[0];
                                self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), mb_ptr, obj_ptr, offset as i32);
                            } else {
                                self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), zero, obj_ptr, offset as i32);
                            }
                        }
                        ...
                        if let Some(&init_id) = self.context.funcs.get(&init_name) {
                            let local_init = self.context.module.declare_func_in_func(init_id, self.builder.func);
                        ...
                        let mut sorted_fields: Vec<_> = layout.fields.iter().map(|(k, v)| (k.clone(), v.clone())).collect();
                        ...
                        for (i, arg) in args.iter().enumerate() {
                            if let Some((_, (offset, _))) = sorted_fields.get(i) {
                                ...
                                self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), arg_val, obj_ptr, *offset as i32);
                            }
                        }
                    ...
                    if let Some(obj_name) = obj_name_opt {
                        if self.context.enum_layouts.contains_key(&obj_name) {
                            ...
                            let func_id = self.context.funcs.get(&constructor_name)
                                .unwrap_or_else(|| panic!("Enum constructor {} not found", constructor_name));
                            let local_callee = self.context.module.declare_func_in_func(*func_id, self.builder.func);
                            
                            let arg_vals = self.translate_args(args)?;
                            
                            let call = self.builder.ins().call(local_callee, &arg_vals);
                            return Ok(self.builder.inst_results(call)[0]);
                        } else if self.context.class_layouts.contains_key(&obj_name) || self.context.struct_layouts.contains_key(&obj_name) {
                            // STATIC METHOD CALL!
                            let static_method_name = format!("{}_{}", obj_name, property);
                            let func_id = self.context.funcs.get(&static_method_name)
                    ...
                    let (m_offset, is_actor) = if let VarType::Object(type_name) = &obj_type {
                        let layout = self.context.class_layouts.get(type_name)
                        ...
                        let layout = self.context.class_layouts.values().find(|l| l.methods.contains_key(property))
                            .unwrap_or_else(|| panic!("Method {} not found in any class layout", property));
                        (*layout.methods.get(property).unwrap(), false)
                    };
                    
                    let vtable_ptr = self.builder.ins().load(ptr_ty, cranelift::prelude::MemFlagsData::new(), obj_ptr, 8);
                    let method_ptr = self.builder.ins().load(ptr_ty, cranelift::prelude::MemFlagsData::new(), vtable_ptr, m_offset as i32);
                    ...
                    if is_actor {
                        let promise_create_id = *self.context.funcs.get("__pace_promise_create").unwrap();
                        let local_promise_create = self.context.module.declare_func_in_func(promise_create_id, self.builder.func);
                        let promise_call = self.builder.ins().call(local_promise_create, &[]);
                        let promise_ptr = self.builder.inst_results(promise_call)[0];
                        
                        let layout = self.context.class_layouts.get(if let VarType::Object(name) = &obj_type { name } else { unreachable!() }).unwrap();
                        let mb_offset = layout.fields.get("__mailbox").unwrap().0;
                        let mailbox_ptr = self.builder.ins().load(types::I64, cranelift::prelude::MemFlagsData::new(), obj_ptr, mb_offset as i32);
                        ...
                        for (i, val) in arg_vals.iter().enumerate() {
                            self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), *val, tuple_ptr, (i * 8) as i32);
                        }
                        
                        let mb_send_id = *self.context.funcs.get("__pace_mailbox_send").unwrap();
                        let local_mb_send = self.context.module.declare_func_in_func(mb_send_id, self.builder.func);
                        self.builder.ins().call(local_mb_send, &[mailbox_ptr, method_ptr, tuple_ptr, promise_ptr]);
                        ...
                        let sig_ref = self.builder.import_signature(sig);
                        let call = self.builder.ins().call_indirect(sig_ref, method_ptr, &arg_vals);
            ...
            Expr::MemberAccess { object, property, computed_class: _, is_static_operator: _ } => {
                ...
                if let Some(obj_name) = obj_name_opt {
                    ...
                    if let Some(&(data_id, ref f_ty)) = maybe_static_field {
                        ...
                        let data_ref = self.context.module.declare_data_in_func(data_id, self.builder.func);
                        let addr = self.builder.ins().symbol_value(ptr_ty, data_ref);
                        let val = self.builder.ins().load(f_ty.to_cranelift_type(), cranelift::prelude::MemFlagsData::new(), addr, 0);
                ...
                let val = self.builder.ins().load(f_ty.to_cranelift_type(), cranelift::prelude::MemFlagsData::new(), obj_ptr, f_offset as i32);
            ...
            Expr::Closure { .. } => {
                ...
                for (i, (name, _)) in captured_vars.iter().enumerate() {
                    ...
                    let val = self.builder.use_var(*var);
                    self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), val, env_ptr, offset as i32);
                }
                
                self.pending_closures.push((fn_name.clone(), expr.clone(), captured_vars));
                
                let mut sig = self.context.module.make_signature();
                sig.params.push(AbiParam::new(self.context.module.target_config().pointer_type())); // env
                ...
                let func_id = self.context.module.declare_function(&fn_name, Linkage::Export, &sig).unwrap();
                let local_func = self.context.module.declare_func_in_func(func_id, self.builder.func);
                let func_ptr = self.builder.ins().func_addr(self.context.module.target_config().pointer_type(), local_func);
                
                self.builder.ins().store(cranelift::prelude::MemFlagsData::new(), func_ptr, env_ptr, 0);
            ...
            Expr::Unwrap(inner) => {
                ...
                let is_null = self.builder.ins().icmp_imm_u(cranelift::prelude::IntCC::Equal, inner_val, 0);
                
                // Trap if null
                let trap_block = self.builder.create_block();
                let cont_block = self.builder.create_block();
                
                self.builder.ins().brif(is_null, trap_block, &[], cont_block, &[]);
                
                self.builder.switch_to_block(trap_block);
                self.builder.seal_block(trap_block);
                self.builder.ins().trap(cranelift::prelude::TrapCode::user(1).unwrap()); // Null pointer dereference
            ...
            Expr::NullCoalesce { left, right } => {
                ...
                let is_null = self.builder.ins().icmp_imm_u(cranelift::prelude::IntCC::Equal, left_val, 0);
                
                let right_block = self.builder.create_block();
                let merge_block = self.builder.create_block();
                self.builder.append_block_param(merge_block, types::I64);
                
                self.builder.ins().brif(is_null, right_block, &[], merge_block, &[cranelift::codegen::ir::BlockArg::Value(left_val)]);
                
                self.builder.switch_to_block(right_block);
                self.builder.seal_block(right_block);
                let right_val = self.translate_expr(right)?;
                self.builder.ins().jump(merge_block, &[cranelift::codegen::ir::BlockArg::Value(right_val)]);
            ...
            Expr::OptionalMemberAccess { object, property } => {
                ...
                let is_null = self.builder.ins().icmp_imm_u(cranelift::prelude::IntCC::Equal, obj_ptr, 0);
                ...
                self.builder.ins().brif(is_null, merge_block, &[cranelift::codegen::ir::BlockArg::Value(zero_val)], access_block, &[]);
                ...
                let val = self.builder.ins().load(types::I64, cranelift::prelude::MemFlagsData::new(), obj_ptr, f_offset as i32);
                ...
                self.builder.ins().jump(merge_block, &[cranelift::codegen::ir::BlockArg::Value(val)]);
