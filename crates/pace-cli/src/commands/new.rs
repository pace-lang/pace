...
"#, name);
    ...
    if is_pkg {
        ...
        std::fs::write(project_path.join("CHANGELOG.md"), "# Changelog\n\n## 0.1.0\n- Initial release\n")
    ...
    std::fs::create_dir_all(&tests_path).map_err(|e| miette::miette!("Failed to create tests directory: {}", e))?;
    std::fs::create_dir_all(&examples_path).map_err(|e| miette::miette!("Failed to create examples directory: {}", e))?;
