    ...
    if let Some(f) = file {
        ...
        if std::path::Path::new("pace.toml").exists() {
            if std::path::Path::new("src/main.pace").exists() {
                ...
                Err(miette!("Default entry point 'src/main.pace' or 'src/<package-name>.pace' not found"))
            }
        } else {
...
pub fn check_sdk_compatibility(manifest: &pace_pkg::manifest::PaceToml) -> Result<()> {
    if let Some(sdk) = &manifest.sdk {
        if let Some(req_str) = sdk.get("pace") {
            ...
            let current_version = semver::Version::parse(current_version_str)
                .map_err(|e| miette::miette!("Invalid current Pace version '{}': {}", current_version_str, e))?;
