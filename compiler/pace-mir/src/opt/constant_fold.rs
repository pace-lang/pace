...
impl ConstantFolder {
    ...
    fn fold_rvalue(&self, rvalue: &mut Rvalue, known_constants: &HashMap<Local, Constant>) {
        if let Rvalue::BinaryOp(op, l1, l2) = rvalue {
            if let (Some(c1), Some(c2)) = (known_constants.get(l1), known_constants.get(l2)) {
                if let Some(folded) = self.eval_binary_op(op, c1, c2) {
...
mod tests {
    ...
    fn test_constant_folding_add() {
        let mut block = BasicBlock {
            statements: vec![
                Statement::Assign(Lvalue::Local(Local(0)), Rvalue::Constant(Constant::Int("5".to_string()))),
                Statement::Assign(Lvalue::Local(Local(1)), Rvalue::Constant(Constant::Int("3".to_string()))),
                Statement::Assign(Lvalue::Local(Local(2)), Rvalue::BinaryOp(BinaryOp::Add, Local(0), Local(1))),
            ],
    ...
    fn test_constant_folding_bool() {
        let mut block = BasicBlock {
            statements: vec![
                Statement::Assign(Lvalue::Local(Local(0)), Rvalue::Constant(Constant::Bool(true))),
                Statement::Assign(Lvalue::Local(Local(1)), Rvalue::Constant(Constant::Bool(false))),
                Statement::Assign(Lvalue::Local(Local(2)), Rvalue::BinaryOp(BinaryOp::And, Local(0), Local(1))),
            ],
            terminator: Some(Terminator::Return(Local(2))),
        };
