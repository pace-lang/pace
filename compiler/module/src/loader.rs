    ...
    fn load_file(&mut self, path: &Path) -> Option<ModuleId> {
        ...
        let source = match fs::read_to_string(path) {
            ...
            Err(e) => {
                ...
                self.errors.push(
                    diagnostics::DiagnosticBuilder::error(
                    ...
                    if let Ok(entries) = std::fs::read_dir(&dir) {
                        for entry in entries.flatten() {
                                ...
                                && let Ok(rel_path) = path.strip_prefix(&std_src)
                            {
                                ...
                                prelude_imports.push(ast::Stmt::new(
                                    ast::StmtKind::Import {
                                        path: self
                                            .session
                                            .interner
        ...
        for stmt in &final_ast {
            ...
            } = &stmt.kind
            {
                let import_path_str = self
                    ...
                    .interner
                    .borrow()
                    .lookup(*import_path_sym)
                    .to_string();
                let clean_path = import_path_str
                    .trim_matches('"')
                    .trim_matches('\'')
                    .to_string();
                ...
                if local_resolved.exists() {
                    resolved_import_path =
                        Some(local_resolved.canonicalize().unwrap_or(local_resolved));
        ...
        self.graph
            .add_module(Module::new(module_id, path.to_path_buf(), final_ast));
