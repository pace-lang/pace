...
impl<'a> TypeChecker<'a> {
    ...
    pub(crate) fn check_expr_with_expected(
        ...
        let (kind, ty) = match &expr.kind {
            ...
            ExprKind::OptionalGet { object, name } => self.check_optional_get_expr(object, *name, expr.span),
            ExprKind::NullCoalesce { left, right } => self.check_null_coalesce_expr(left, right, expr.span),
            ExprKind::NullCoalesceAssign { left, right } => self.check_null_coalesce_assign_expr(left, right, expr.span),
            ExprKind::Array(elements) => self.check_array_expr(elements, expr.span),
            ExprKind::ArrayRepeat { value, count } => self.check_array_repeat_expr(value, count, expr.span),
            ...
            ExprKind::IndexGet { object, index } => self.check_index_get_expr(object, index, expr.span),
            ExprKind::IndexSet { object, index, value } => self.check_index_set_expr(object, index, value, expr.span),
            ExprKind::Get { object, name } => self.check_get_expr(object, *name, expr.span),
            ExprKind::Set { object, name, value } => self.check_set_expr(object, *name, value, expr.span),
            ...
            ExprKind::Call { callee, type_args, arguments } => self.check_call_expr(callee, type_args, arguments, expected_ty, expr.span),
