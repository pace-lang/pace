...
impl TypeChecker {
                ...
                if let pace_ast::Type::Named(id) = &**base {
                    ...
                    if let Some(template) = self.generic_templates.get(&id.name) {
                        let generic_params = match template {
                            ...
                            pace_hir::Decl::Function { generic_params, .. } => generic_params.clone().unwrap_or_default(),
                            _ => vec![],
                        };
    ...
    pub fn check_program(&mut self, program: &Program) -> Result<(), String> {
        ...
        for decl in &program.declarations {
            if let Decl::Class { id, extends, span, .. } = decl {
                if let Some(parent_name) = extends {
                    if let Some(&(parent_id, 1)) = self.named_types.get(parent_name) {
                        if parent_id == *id {
                            self.reporter.report(pace_errors::Diagnostic::error("Class cannot inherit from itself").with_span(*span));
                            return Err("Class cannot inherit from itself".to_string());
                        }
                        self.class_parents.insert(*id, parent_id);
                    } else {
                        self.reporter.report(pace_errors::Diagnostic::error("Class extends a non-class or unknown type").with_span(*span));
        ...
        for &id in &class_ids {
            ...
            while let Some(c_id) = curr {
                if hierarchy.contains(&c_id) {
                    self.reporter.report(pace_errors::Diagnostic::error("Circular inheritance detected").with_span(pace_span::Span::new(0, 0)));
        ...
        for decl in &program.declarations {
            if let Decl::Function { id, generic_params, params, return_type, .. } = decl {
                if generic_params.is_some() { continue; }
                let mut param_tys = Vec::new();
                for (_, _, pty) in params {
        ...
        let mut class_methods: HashMap<HirId, Vec<(String, Ty, String, bool, pace_span::Span)>> = HashMap::new();
        for decl in &program.declarations {
            if let Decl::Class { id, methods, .. } = decl {
                let mut cm = Vec::new();
                for m in methods {
                    if let Decl::Function { name, is_override, span, .. } = m {
                    ...
                    for (base_name, ty, full_name, is_override, span) in methods {
                        if *is_override {
                            if let Some(pos) = vtable.iter().position(|(n, _, _)| n == base_name) {
                                ...
                                if let (Ty::Function(base_params, base_ret), Ty::Function(new_params, new_ret)) = (base_ty, ty) {
        ...
        for (id, name, span) in &self.declared_bindings {
            ...
            if !self.used_bindings.contains(&id) && !name.starts_with('_') && name != "main" && name != "self" && !name.ends_with("_init") && !is_compiler_generated {
    ...
    pub fn check_decl(&mut self, decl: &Decl) -> Result<(), String> {
        match decl {
            ...
            Decl::Function { id, name, generic_params, params, return_type, body, span, is_static, is_override: _, .. } => {
                if generic_params.is_some() {
                    self.generic_templates.insert(name.clone(), decl.clone());
                    self.generic_templates_by_id.insert(*id, name.clone());
                    return Ok(());
                ...
                if name.contains('_') && name != "main" && !name.ends_with("_init") {
                    ...
                    if !is_compiler_generated {
                        ...
                        let name_span = pace_span::Span::new(span.start + offset, span.start + offset + name.len());
                ...
                for (param_id, param_name, pty) in params {
                    ...
                    self.declared_bindings.push((*param_id, param_name.clone(), *span));
        ...
        match expr {
            ...
            Expr::Super(span) => {
                ...
                for (id, ty) in &self.env {
                    if let Some((_, name, _)) = self.declared_bindings.iter().find(|(b_id, _, _)| b_id == id) {
                        if name == "self" {
                            if let Ty::Class(class_id) = ty {
                                if let Some(&parent_id) = self.class_parents.get(class_id) {
                                    return Ok(Ty::Class(parent_id));
                                } else {
                                    self.reporter.report(pace_errors::Diagnostic::error("Cannot use 'super' in a class with no parent").with_span(*span));
                ...
                self.reporter.report(pace_errors::Diagnostic::error("Cannot use 'super' outside of a class method").with_span(*span));
            ...
            Expr::Call { callee, args, span } => {
                ...
                if let Expr::Ident(id, name, _) = &**callee {
                    if self.generic_templates_by_id.contains_key(id) {
                        ...
                        if let pace_hir::Decl::Function { generic_params, params, .. } = template {
                            ...
                            for (param_name, _) in &generic_params {
                                if let Some(ty) = inferred_args.get(param_name) {
                                    let ty_str = format!("{:?}", ty).replace(" ", "").replace("(", "_").replace(")", "_").replace(":", "_");
                                    ast_args.push(pace_ast::Type::Named(pace_ast::Ident { name: ty_str, span: *span }));
                                }
                            }
                ...
                if let Ty::Struct(id) = callee_ty {
                    ...
                    let mut def_map: std::collections::HashMap<_, _> = def_fields.into_iter().map(|(n, t, _)| (n, t)).collect();
                ...
                if let Ty::Function(_, ret_ty) = &callee_ty {
                    if let Ty::Enum(enum_id) = **ret_ty {
                        ...
                        let mut def_map: std::collections::HashMap<_, _> = def_fields.into_iter().map(|(n, t)| (n, self.resolve_type(&t).unwrap_or(Ty::Int))).collect();
                        for (label, fexpr) in args {
                            let fty = self.check_expr(fexpr)?;
                            if let Some(fname) = label {
                                if let Some(expected_ty) = def_map.remove(fname) {
                                    ...
                                    self.reporter.report(Diagnostic::error(format!("Unknown field '{}' in variant instantiation", fname)).with_span(*span).with_code(ErrorCode::UnknownField));
                                    return Err(format!("Unknown field '{}' in variant instantiation", fname));
                                }
                            } else {
                                self.reporter.report(Diagnostic::error("Variant instantiation requires named arguments").with_span(*span).with_code(ErrorCode::InvalidArguments));
                                return Err("Variant instantiation requires named arguments".to_string());
                            }
                        }
                        def_map.retain(|_, ty| !matches!(ty, Ty::Optional(_)));
                        if !def_map.is_empty() {
                            self.reporter.report(Diagnostic::error(format!("Missing fields in variant instantiation: {:?}", def_map.keys())).with_span(*span).with_code(ErrorCode::MissingFields));
                            return Err(format!("Missing fields in variant instantiation: {:?}", def_map.keys()));
                        }
            ...
            Expr::Match { subject, arms, span: _ } => {
                ...
                for arm in arms {
                    ...
                    if ret_ty.is_none() {
                        ...
                        self.reporter.report(Diagnostic::error(format!("Match arms have incompatible types: {:?} and {:?}", ret_ty.as_ref().unwrap(), arm_ty))
                            .with_span(arm.span)
                            .with_code(ErrorCode::TypeMismatch));
                    }
                }
    ...
    pub fn instantiate_generic(&mut self, template_name: &str, args: &[pace_ast::Type]) -> Result<Ty, String> {
        ...
        for (i, (param_name, default_ty)) in generic_params.iter().enumerate() {
            if i < args.len() {
                ...
                return Err(format!("Missing generic argument for {} and no default provided", param_name));
            }
        }
        
        if final_args.len() != generic_params.len() {
            return Err(format!("Expected {} generic arguments, got {}", generic_params.len(), final_args.len()));
        }
        ...
        match &mut new_decl {
            ...
            pace_hir::Decl::Function { id, name, params, return_type, generic_params, .. } => {
                *id = self.generate_id();
                *name = mono_name.clone();
                *generic_params = None;
        ...
        if let pace_hir::Decl::Function { id, params, return_type, .. } = &new_decl {
            let mut param_tys = Vec::new();
        ...
        match new_decl {
            ...
            pace_hir::Decl::Function { params, return_type, .. } => {
                let mut param_tys = Vec::new();
                for (_, _, ty) in params {
                    param_tys.push(self.resolve_type(&ty)?);
                }
