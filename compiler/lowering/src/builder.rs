...
mod tests {
    ...
    fn test_lower_assignment() {
        ...
        // let x = 10 + 5;
        let stmt = TypedStmt::new(
            TypedStmtKind::Let {
                name: session.interner.borrow_mut().intern("x"),
                type_annotation: None,
                initializer: Some(TypedExpr::new(
                    TypedExprKind::Binary(
                        Box::new(TypedExpr::new(
                            TypedExprKind::Integer(10),
                            session::types::Type::Int,
                            make_span(),
                        )),
                        BinaryOp::Add,
                        Box::new(TypedExpr::new(
                            TypedExprKind::Integer(5),
                            session::types::Type::Int,
                            make_span(),
                        )),
                    ),
        ...
        let stmt = TypedStmt::new(
            TypedStmtKind::If {
                condition: TypedExpr::new(
                    TypedExprKind::Boolean(true),
                    session::types::Type::Boolean,
                    make_span(),
                ),
                then_branch: Box::new(TypedStmt::new(
                    TypedStmtKind::Block(vec![TypedStmt::new(
                        TypedStmtKind::Let {
                            name: session.interner.borrow_mut().intern("x"),
                            type_annotation: None,
                            initializer: Some(TypedExpr::new(
                                TypedExprKind::Integer(1),
                                session::types::Type::Int,
                                make_span(),
                            )),
                        },
