...
impl<'a> MirBuilder<'a> {
    pub(crate) fn lower_expr(&mut self, expr: &TypedExpr) -> Value {
        match &expr.kind {
            ...
            TypedExprKind::InterpolatedString(pieces) => self.lower_interpolated_string_expr(pieces),
            ...
            TypedExprKind::ArrayRepeat { value, count } => self.lower_array_repeat_expr(value, count),
            ...
            TypedExprKind::OptionalGet { object, name } => self.lower_optional_get_expr(object, *name),
            TypedExprKind::NullCoalesce { left, right } => self.lower_null_coalesce_expr(left, right),
            TypedExprKind::NullCoalesceAssign { left, right } => {
                self.lower_null_coalesce_assign_expr(left, right)
            }
            TypedExprKind::Set { object, name, value } => self.lower_set_expr(object, *name, value),
